...
Host agents must authenticate with Central using an enrollment token (generated via `portforge central generate-token`). This token issues a permanent, host-specific cryptographic credential used to sign subsequent snapshot transmissions and requests. An attacker cannot submit data for a host without its specific credential.
