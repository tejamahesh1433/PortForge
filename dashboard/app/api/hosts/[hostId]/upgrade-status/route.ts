...
/**
 * Server-side host upgrade-status proxy.
 *
 * GET /api/hosts/{hostId}/upgrade-status is admin-gated on Central.
...
export async function GET(
  ...
  const url = new URL(
    `/api/hosts/${hostId}/upgrade-status`,
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Could not reach Central at ${centralBase} to get host upgrade status.`,
  ...
  const detail =
    ...
    typeof (payload as { detail: unknown }).detail === "string"
      ...
      : `Central returned ${response.status} while fetching host upgrade status.`;
