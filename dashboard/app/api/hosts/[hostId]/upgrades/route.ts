...
/**
 * Server-side upgrade creation proxy.
 *
 * POST /api/hosts/{hostId}/upgrades requires admin authorization.
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Could not reach Central at ${centralBase} to create the upgrade.`,
  ...
  const detail =
    ...
    typeof (payload as { detail: unknown }).detail === "string"
      ...
      : `Central returned ${response.status} while creating the upgrade.`;
