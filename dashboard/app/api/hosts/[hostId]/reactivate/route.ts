...
/**
 * Server-side Reactivate proxy.
 *
 * Browser never holds PORTFORGE_ADMIN_BOOTSTRAP_TOKEN. This route attaches
 * it when calling Central POST /api/hosts/{id}/reactivate.
 */
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Could not reach Central at ${centralBase} to reactivate the host.`,
  ...
  const detail =
    ...
    typeof (payload as { detail: unknown }).detail === "string"
      ...
      : `Central returned ${response.status} while reactivating the host.`;
