...
function makeRequest(body: Record<string, unknown> = {}) {
  return new Request(`http://localhost/api/hosts/${HOST_ID}/decommission`, {
    method: "POST",
...
const decomissionedHostBody = JSON.stringify({
  id: HOST_ID,
  hostname: "lab-mac",
  lifecycle_state: "DECOMMISSIONED",
});

describe("POST /api/hosts/[hostId]/decommission", () => {
  beforeEach(() => {
    vi.unstubAllEnvs();
    vi.unstubAllGlobals();
  });
  ...
  it("forwards Central 200 with server-side admin Authorization", async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(decomissionedHostBody, {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }),
    );
    ...
    const response = await POST(makeRequest({ reason: "retiring server" }), { params });
    expect(response.status).toBe(200);
    expect(fetchMock).toHaveBeenCalledOnce();
    const [url, init] = fetchMock.mock.calls[0] as [string | URL, RequestInit];
    expect(String(url)).toBe(
      `http://central.test/api/hosts/${HOST_ID}/decommission`,
    );
    expect(init.method).toBe("POST");
    expect(init.headers).toMatchObject({ Authorization: "Bearer admin-secret" });
    const result = await response.json();
    expect(result.lifecycle_state).toBe("DECOMMISSIONED");
  });
