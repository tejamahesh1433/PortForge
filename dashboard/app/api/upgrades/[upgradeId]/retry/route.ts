...
/**
 * Server-side upgrade retry proxy.
 *
 * POST /api/upgrades/{upgradeId}/retry requires admin authorization.
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Could not reach Central at ${centralBase} to retry the upgrade.`,
  ...
  const detail =
    ...
    typeof (payload as { detail: unknown }).detail === "string"
      ...
      : `Central returned ${response.status} while retrying the upgrade.`;
