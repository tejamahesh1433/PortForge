...
function makeRequest() {
  return new Request(`http://localhost/api/upgrades/${UPGRADE_ID}/retry`, {
    method: "POST",
...
describe("POST /api/upgrades/[upgradeId]/retry", () => {
  beforeEach(() => {
    vi.unstubAllEnvs();
    vi.unstubAllGlobals();
  });
  ...
  it("forwards retry to Central with server-side admin Authorization", async () => {
    vi.stubEnv("PORTFORGE_ADMIN_BOOTSTRAP_TOKEN", "admin-secret");
    ...
    expect(String(url)).toBe(`http://central.test/api/upgrades/${UPGRADE_ID}/retry`);
  ...
  it("does not leak the admin token in the response body on Central error", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ detail: "Upgrade is already in a terminal state." }), {
          status: 409,
          headers: { "Content-Type": "application/json" },
        }),
      ),
    ...
    const body = await response.json();
    expect(body.detail).toMatch(/terminal/i);
