...
/**
 * Server-side upgrade rollback proxy.
 *
 * POST /api/upgrades/{upgradeId}/rollback requires admin authorization.
...
export async function POST(
  ...
  const url = new URL(
    `/api/upgrades/${upgradeId}/rollback`,
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Could not reach Central at ${centralBase} to rollback the upgrade.`,
  ...
  const detail =
    ...
    typeof (payload as { detail: unknown }).detail === "string"
      ...
      : `Central returned ${response.status} while rolling back the upgrade.`;
