...
/**
 * Server-side upgrade status proxy.
 *
 * GET /api/upgrades/{upgradeId}/status is admin-gated on Central.
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Could not reach Central at ${centralBase} to get upgrade status.`,
  ...
  const detail =
    ...
    typeof (payload as { detail: unknown }).detail === "string"
      ...
      : `Central returned ${response.status} while fetching upgrade status.`;
